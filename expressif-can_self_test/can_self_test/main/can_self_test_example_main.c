...
{
    twai_message_t tx_msg = {.data_length_code = 1, .identifier = MSG_ID, .flags = TWAI_MSG_FLAG_SELF};
...
static void twai_control_task(void *arg)
{
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++) {
        ...
        ESP_ERROR_CHECK(twai_stop());                //Stop the CAN Driver
...
void app_main()
{
    ...
    xTaskCreatePinnedToCore(twai_control_task, "CAN_ctrl", 4096, NULL, CTRL_TSK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(twai_receive_task, "CAN_rx", 4096, NULL, RX_TASK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(twai_transmit_task, "CAN_tx", 4096, NULL, TX_TASK_PRIO, NULL, tskNO_AFFINITY);
