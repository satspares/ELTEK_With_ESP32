...
void sendLogin(uint32_t ID, uint8_t extd, uint8_t data[]) {
    ...
    loginFrame.data_length_code = 6;
    memcpy(loginFrame.data, data, 6);
...
bool processFrame(uint32_t identifier) {
    ...
    if (((identifier & 0xffff0000) == 0x05000000) && (!serialNumberRXed)) { // || (rxFrame.data[0] == 0x1b/5b)) ){
        #ifdef myDebug
        Serial.println("---------------");
        Serial.print("rxFrame.data[0] ");
        Serial.println(rxFrame.data[0],HEX);  // my known values 0x1b 0x5b sent before serial number (version?)
        #endif
        frame_array[0].psuid_string = "";
        for (int i = 0; i < 6; ++i) {
            serialNumber[i] = rxFrame.data[i + 1];            
            snprintf(serial_out, 3, "%02X", serialNumber[i]);
...
void setSession(uint32_t ID, uint16_t voltage, uint16_t hi_voltage, uint16_t current) {
    ...
    uint16_t scaledCurrent = current / 10;
    sessionFrame.identifier = ID;
    sessionFrame.extd = 1;
    sessionFrame.data_length_code = 8;
    sessionFrame.data[0] = lowByte(scaledCurrent); sessionFrame.data[1] = highByte(scaledCurrent);
...
void saveFrame(uint8_t frameCopy[], bool current_limit, bool ramping) {
    ...
    frame_array[0].voltage_input  = bytes_to_u16(frameCopy[6], frameCopy[5]);
    frame_array[0].voltage_output = bytes_to_u16(frameCopy[4], frameCopy[3]) / 100.0f;
    frame_array[0].current_output = bytes_to_u16(frameCopy[2], frameCopy[1]) / 10.0f;
    frame_array[0].power_output   = (uint16_t)(frame_array[0].current_output
                                             * frame_array[0].voltage_output);
...
void setDefaultVoltage(uint16_t setVoltage) {  // before setSession is active
    ...
    defaultFrame.data[0] = 0x29; //from manufacturer's tool
    defaultFrame.data[1] = 0x15; //from manufacturer's tool
...
static inline uint16_t bytes_to_u16(uint8_t hi, uint8_t lo) {
    return (uint16_t)((uint16_t)hi * 256 + lo);
}